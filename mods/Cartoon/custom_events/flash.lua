...
function onCreatePost()
	...
	for i = 0, getProperty('eventNotes.length')-1 do
		...
		if getPropertyFromGroup('eventNotes', i, 'event') == 'flash' then
			...
			--get the next flash event (it causes less lag because it doesn't need to get every single note until the end of the song for each event)
			for iii = 0, getProperty('eventNotes.length')-1 do
			
				if not (iii == i) and getPropertyFromGroup('eventNotes', iii, 'event') == 'flash' then
