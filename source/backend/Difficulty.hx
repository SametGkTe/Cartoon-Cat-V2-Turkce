...
class Difficulty
{
    ...
    public static function getDisplayString(num:Null<Int> = null):String
    {
        var diffName:String = getString(num);

        if (diffName == 'Easy') return 'Kolay';
        if (diffName == 'Normal') return 'Normal';
        if (diffName == 'Hard') return 'Zor';
        return diffName;
    }
